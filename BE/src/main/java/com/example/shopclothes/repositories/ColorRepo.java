...
public interface ColorRepo extends JpaRepository<Color, Long> {
    ...
    @Query(value = "select cl.date_create, cl.date_update, cl.id, cl.code, cl.ghi_chu, cl.name, " +
            "cl.status from color cl join product_detail on cl.id = product_detail.id_col where id_col = ?1", nativeQuery = true)
    List<Color> findByIdProduct(Long id);
