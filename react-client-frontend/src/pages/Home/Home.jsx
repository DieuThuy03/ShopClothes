    ...
    const getProductHomePageByProducts = async () => {
        ...
        await ProductService.getAll(pagination.current - 1)
            .then(response => {
                ...
                const productFormat = listProduct.map(products => ({
                    key: products.id,
                    id: products.id,
                    code: products.code,
                    ...
                    deleted: String(products.status),
                }));
                        ...
                        <Col xs={24} sm={12} md={8} lg={6} key={item.id} >
                            <Link to={`${path_name.product_detail}/${item.id}`}>
                                <Card
                                    ...
                                    cover={<Image alt="example" src={item.imageLink} />}
                                >
                                    <Meta title={item.name}
                                        description={<span style={{ color: 'red', fontWeight: '550' }}>{formatCurrency(item.price)}</span>}
                                    />
                                </Card>
                            </Link>
                        </Col>
